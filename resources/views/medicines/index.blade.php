    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8  ">
        ...
        <div class="w-full bg-gray-800 py-4 px-7 rounded-md " >
            <div class="mb-8 flex items-center justify-between gap-8">
                <div>
                    <h6 class="font-sans antialiased font-bold text-base md:text-lg lg:text-xl text-white ">Medication list</h6>
                    <p class="font-sans antialiased text-base text-gray-300 mt-1">See information about all Patients</p>
                </div>
            ...
            <div class="mt-4 w-full overflow-hidden rounded-lg border border-stone-200">
                <table class="w-full">
                    ...
                    <tr>
                        ...
                        <th class="px-2.5 py-2 text-start font-medium">Name of the med</th>
                        <th class="px-2.5 py-2 text-start font-medium">Dosage</th>
                        ...
                        <tr class="border-b border-stone-200 last:border-0">
                            <td class="p-3">
                                ...
                                <div class="flex items-center gap-3">
                                    <img src="{{ asset($med->patient->logo) }}" alt="{{ $med->patient->FName }}" class="inline-block object-cover object-center w-8 h-8 rounded" />
                                    <div class="flex flex-col">
                                        <small class="font-sans antialiased text-sm text-current"> {{ $med->patient->FName }} {{ $med->patient->LName }}</small>
                                        <small class="font-sans antialiased text-sm text-current opacity-70">{{ $med->patient->email }}</small>
                                    </div>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="flex flex-col">
                                    ...
                                    <small class="font-sans antialiased text-sm text-current opacity-70">{{ $med->name }}</small>
                                </div>
                            </td>
                            </a>
                            <td class="p-3">
                                <div class="flex flex-col">
                                    {{--                                <small class="font-sans antialiased text-sm text-current">Manager</small>--}}
                                    <small class="font-sans antialiased text-sm text-current opacity-70">{{ $med->dosage ?? '—' }}</small>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="w-max">
                            ...
                            <td class="p-3">
                                <div class="flex flex-col">
                                    <small class="font-sans antialiased text-sm text-current">{{ $med->description }}</small>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="flex flex-col">
                                    {{--                                <small class="font-sans antialiased text-sm text-current">Manager</small>--}}
                                    <small class="font-sans antialiased text-sm text-current opacity-70">{{ $med->created_at->format('Y-m-d') }}</small>
                                </div>
                            </td>
                            <td class="p-3">
                                ...
                                @if(auth()->id()===$med->prescribed_by)
                                    <form method="POST" action="{{ route('medicines.destroy',$med)}}" onsubmit="return confirm('Are you sure you want to delete this medication?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
